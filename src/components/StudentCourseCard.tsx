...
export function StudentCourseCard(): ReactElement {
	...
	return (
		...
		// 	<h2 className="g-card-header">{studentData?.course.courseName}</h2>
		// 	<p className="g-text-subheading">Your current course</p>
		// 	<h3 className="g-list-item-header">Participants</h3>
		// 	<p className="n-students g-list-item-text">{studentData?.course.users.length} students enrolled</p>
		// 	<CourseDetailsStudentsDropdown students={studentData?.course.users} />
		// 	<h3 className="g-list-item-header">Modules</h3>
		// 	<ul className="g-list">
		// 		{studentData?.course.modules.map((module) => (
